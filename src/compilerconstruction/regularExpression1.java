...
//        }
    ...
    static LinkedHashSet<String> PositiveClosure(String rExp, LinkedHashSet<String> rSet) {
        String mid = " ";
        String last = " ";
        String appearsBefore = " ";
        ...
        if (rExp.contains("+")) {
            unionOperatorForPositive(rExp, rSet);
        }
   ...
   static LinkedHashSet<String> unionOperatorForPositive(String rExp, LinkedHashSet<String> rSet) {
        int i = rExp.indexOf('+');
        ...
        if (startS.contains("(")) {
            rSet = bracketStringForPositive(startS, rSet, rExp.indexOf('('));
        } else {
            rSet = PositiveClosure(startS, rSet);
        }

        if (endS.contains("(")) {
            rSet = bracketStringForPositive(endS, rSet, rExp.indexOf('('));
        } else {
            rSet = PositiveClosure(endS, rSet);
        }

        return rSet;
    }

    static LinkedHashSet<String> bracketStringForPositive(String rExp, LinkedHashSet<String> rSet, int j) {
        String s = " ", s1, s2, s3 = " ", s4 = " ";

        if (Pattern.matches("[(]+[0-1]*[\\+]+[0-1]*[)]+[\\$]", rExp)) {
            s1 = rExp.substring(rExp.indexOf('(') + 1, rExp.indexOf('+'));
            s2 = rExp.substring(rExp.indexOf('+') + 1, rExp.indexOf(')'));
            s3 += s1 + s2;
            s4 += s2 + s1;
            ...
            rSet.add(s4.trim());
            rSet.add(s4.trim() + s4.trim());
            System.out.println("S --> "+s1+"A|"+s2+"A");
            System.out.println("A --> ϵ|"+s1+"|"+s2);
            ...
            if (s1.contains("$")) {
                rSet.addAll(PositiveClosure(s1.trim(), rSet));
            }
            if (s2.contains("$")) {
                rSet.addAll(PositiveClosure(s2.trim(), rSet));
            } else {
                rSet = PositiveClosure(s1.trim(), rSet);
                rSet.addAll(PositiveClosure(s2.trim(), rSet));
            }
            if(rExp.charAt(rExp.length()-1) == '$')
                System.out.println("S --> "+s1.replaceAll("[0-1]?[\\$]+","A")+"|"+s2.replaceAll("[0-1]?[\\$]+","A"));
                ...
                System.out.println("S --> "+s1.replaceAll("[0-1]?[\\$]+","A")+"|"+s2.replaceAll("[0-1]?[\\$]+","A"));
            if(r1.length() > 1 && r2.length()>1 && r1.contains("A") && r2.contains("A"))
                System.out.println("A --> "+r1.substring(r1.indexOf('A')-1, r1.indexOf('A')+1)+'|'+r2.substring(r2.indexOf('A')-1, r2.indexOf('A')+1));
           
            else if(r1.length()>1 && r1.contains("A"))
                System.out.println("A --> "+r1.substring(r1.indexOf('A')-1, r1.indexOf('A')+1));
           
            else
                System.out.println("A --> "+r2.substring(r2.indexOf('A')-1, r2.indexOf('A')+1));
...
class RegularExpressionDriver1 {
    ...
    public static void main(String args[]) {
        ...
        if (rExp.contains("(") && rExp.contains("*")) {
            ...
            rSet = bracketStringForPositive(rExp, rSet, rExp.indexOf('('));
        }
        else {
            if (!rExp.contains("+") && !rExp.contains("*") && !rExp.contains("$")) {
                ...
                rSet = PositiveClosure(rExp, rSet);
            } else if (rExp.contains("$")) {
                String s1,s2;
                rSet = PositiveClosure(rExp, rSet);
                if(rExp.length()>2){
                ...
                String s = rExp.substring(rExp.indexOf('$')-1, rExp.indexOf('$')+1);
                System.out.println("A --> "+rExp.substring(rExp.indexOf('$')-1, rExp.indexOf('$'))+"|"+s.replace('$', 'A'));
                }
        
                else{
                    System.out.println("S --> "+rExp.charAt(0)+"A");
                    System.out.println("A --> ϵ|"+rExp.charAt(0)+"A");
